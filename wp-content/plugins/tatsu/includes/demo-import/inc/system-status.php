...
if ( ! function_exists( 'be_stat_green_or_red' ) ) {
	function be_stat_green_or_red( $condation, $bigger ) {
		$html = '';
		if ( strpos( strtoupper( $condation ), 'G' ) !== false ) {
			$condation = intval( $condation ) * 1024;
		}

		if ( strpos( strtoupper( $condation ), 'M' ) !== false ) {
			$condation = intval( $condation );
		}
...
if( !function_exists( 'tatsu_demos_system_status_tpl' ) ) {
	function tatsu_demos_system_status_tpl() {
		global $wpdb;
	?>
	...
	$muplugins = TGM_Plugin_Activation::get_instance();
		if(!empty($muplugins->plugins) && sizeof( $muplugins->plugins ) > 0 ) {
	...
	<?php 
	echo be_stat_green_or_red( PHP_VERSION, '5.6.0' ); ?>PHP Version:              <?php echo PHP_VERSION . "\n"; ?>
	<?php 
	echo be_stat_green_or_red( ini_get( 'post_max_size' ), '256' ); ?>PHP Post Max Size:        <?php echo ini_get( 'post_max_size' ) . "\n"; ?>
	<?php 
	echo be_stat_green_or_red( ini_get( 'max_execution_time' ), '300' ); ?>PHP Time Limit:           <?php echo ini_get( 'max_execution_time' ) . "\n"; ?>
	<?php 
	echo be_stat_green_or_red( ini_get( 'max_input_vars' ), '1200' ); ?>PHP Max Input Vars:       <?php echo ini_get( 'max_input_vars' ) . "\n"; ?>
	PHP Safe Mode:            <?php echo ini_get( 'safe_mode' ) ? "Yes" : "No\n"; ?>
	<?php 
	echo be_stat_green_or_red( ini_get( 'memory_limit' ), '256' ); ?>PHP Memory Limit:         <?php echo ini_get( 'memory_limit' ) . "\n"; ?>
