...
class ExponentImporter {
  ...
  private static $demo_names;
    ...
    public function __construct() {
        ...
        $this->wpforms_data = $this->demo_files_path . $this->wpforms_data_file_name;
    ...
    */
    ...
    /**
     * Process Import WP Forms 
     */
    function process_wpforms_import_file( $file = ''){
      if(empty($file)) { 
        $file = $this->wpforms_data;
      }
      // File exists?
    	if ( ! file_exists( $file )) {
    		wp_die(
    			'WP Forms Import file OR Plugin could not be found. ',
    			'',
    			array( 'back_link' => true )
    		);
    	}
      ...
      if(!empty($data) && is_array($data)){
        foreach ( $data as $form ) {
          if(!empty($form)){
            ...
            $new_id = wp_insert_post( array('post_title'   => $title,
                                            'post_status'  => 'publish',
                                            'post_type'    => 'wpforms',
                                            'post_excerpt' => $desc ));
            if ( $new_id ) {
              $form['id'] = $new_id;
              $post_content = function_exists('wpforms_encode')?wpforms_encode($form):wp_slash( wp_json_encode( $form ) );
              wp_update_post(array('ID' => $new_id,'post_content' => $post_content));
            }
          }
        }
      }else{
        wp_die(
    			'WP Forms Import file data incorrect. ',
    			'',
    			array( 'back_link' => true )
    		);
      }
