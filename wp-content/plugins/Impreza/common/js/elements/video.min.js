(function($,undefined){var _window=window,_document=document;_window.$us.YTPlayers=_window.$us.YTPlayers||{};"use strict";$us.wVideo=function(container){var self=this;self.$container=$(container);self.$videoH=$('.w-video-h',self.$container);self.$videoTag='';self.cookieName=self.$container.data('cookie-name');self.isWithOverlay=self.$container.hasClass('with_overlay');if(!self.cookieName&&!self.isWithOverlay){return}
...
self.$videoH.html(data.player_html);self.$videoTag=self.getVideo();if(!data.player_api&&self.$videoTag){self.$videoTag.play()}},getVideo:function(){return this.$videoH.find('video')[0]||!1}});$.fn.wVideo=function(options){return this.each(function(){$(this).data('wVideo',new $us.wVideo(this,options))})};$(function(){$('.w-video').wVideo()})})(jQuery)